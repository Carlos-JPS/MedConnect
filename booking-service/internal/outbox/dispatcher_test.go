...
func (s *fakeStore) FetchPendingOutboxEvents(_ context.Context, _ int) ([]Event, error) {
	return s.events, nil
}
...
func (s *fakeStore) MarkOutboxFailed(_ context.Context, eventID string, nextAttemptAt time.Time, lastError string) error {
...
func TestDispatchBatchSchedulesRetryOnPublishError(t *testing.T) {
	...
	dispatcher := NewDispatcher(
		store,
		publisher,
		DispatcherConfig{BatchSize: 10, InitialRetryDelay: time.Second, MaxRetryDelay: 30 * time.Second},
		nil,
	)
